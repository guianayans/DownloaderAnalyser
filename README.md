...
|-------|----------------|
| **Repository name** | `downloader` |
| **Description** | Painel web para explorar arquivos no servidor e baixar via wget, curl, aria2, git e Hugging Face — com histórico, atalhos e análise de armazenamento. |
| **Visibility** | Public (ou Private, se preferir) |

### Primeira vez — criar o repositório e enviar

1. No GitHub: **New repository** → nome `downloader` → **Create repository** (sem README, se você já tiver um local).

2. Na pasta do projeto no seu Mac:

```bash
cd /Users/yanguimaraesviana/Desktop/ComfyUI-0.23.0/downloader

git init
git branch -M main
git remote add origin https://github.com/SEU-USUARIO/downloader.git
```

3. A **URL** do repositório aparece na página do GitHub, botão verde **Code** → copie HTTPS ou SSH, por exemplo:

```
https://github.com/SEU-USUARIO/downloader.git
```
...
```bash
git add -A
git commit -m "Initial commit: Downloader web panel"
git push -u origin main
```

### Atualizações depois (seu fluxo habitual)

Sempre na pasta do projeto (`downloader/`):
...
Se o repositório já existir no GitHub e você clonou de lá, pule `git init` e `remote add` — use só pull → add → commit → push.

### Se o app estiver dentro de um monorepo maior

Se preferir publicar **só** a pasta `downloader` como repositório independente, use os comandos acima **dentro de** `downloader/`, não na raiz do ComfyUI.
